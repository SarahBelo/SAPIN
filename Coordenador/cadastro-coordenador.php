...
<body>
    ...
    <div id="fundo">
        <form>
            ...
            <br>
            <input type="email" name="txtConfEmail" id="txtConfEmail" required>
            <br>
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $('#btnCadastrar').on('click', async function(e){
                ...
                try {
                    const response = await fetch('../Controller/Coordenador/CadastroCoordenador.php', config);
                    const data = await response.json();

                    if(data.status == 1){
                        alert('Coordenador cadastrado com sucesso');
                        window.location.href = `nav-coordenador.php?id=${data.id}`;
                    } else {
                        alert('Erro ao cadastrar coordenador. Verifique as informações.');
                    }
                } catch (error) {
