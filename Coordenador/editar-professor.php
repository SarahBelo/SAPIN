...
<script type="text/javascript" charset="utf-8">
    async function carregarDadosProfessor(id){
        ...
        const request = await fetch('../Controller/Professor/ListarIDProfessor.php', config);
   ...
   const request = await fetch('../Controller/Professor/EditarProfessor.php', config);
   const resultado = await request.json();
   if(resultado.status === 1){
       alert('Dados alterados');
       window.location.href = `nav-professor-coordenador.php?id=${id}`;
   } else{
