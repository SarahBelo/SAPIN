...
<head>
    ...
    <title>Tabela de Coordenadores</title>
...
<body>
    ...
    <a class="btn-table-container" href="cadastro-coordenador.php?id=<?php echo $_SESSION['user_id']; ?>">
        <button class="btn btn-primary" id="cadastrarCoordenador">Cadastrar Coordenador</button>
    </a>
...
<script type="text/javascript" charset="utf-8">
  async function carregarDadosCoordenador() {
    const request = await fetch('../Controller/Coordenador/ListarCoordenador.php', { method: 'GET' });
    const response = await request.json();
    const coordenador = document.getElementById('bodyCoordenador');
    for (const item of response.dados) {
        let statusCoordenador = item.status_coordenador != 0 ? 'Ativo' : 'Desativado';
      coordenador.innerHTML += `
            <tr>
                ...
                <td>${item.email}</td>
                <td class="acoes">
                        <button class="btn btn-primary editarCoordenador" onclick="editarCoordenador(${item.id})" id="" data-id="${item.id}">Editar</button>
                    </td>
                    <td class="acoes">
                        <select name="status" id="status-${item.id}" onchange="statusCoordenador(${item.id})">
                            <option value="select" select disabled>${statusCoordenador}</option>
                            <option value="1" ${statusCoordenador === 'Ativo' ? 'selected' : ''}>Ativo</option>
                            <option value="0" ${statusCoordenador === 'Desativado' ? 'selected' : ''}>Desativado</option>
                        </select>
                    </td>
                </tr>`;
    }
  ...
  function editarCoordenador(id) {
    window.location.href = `editar-coordenador.php?id=${id}`;
  }

  async function statusCoordenador(id) {
        ...
        body: JSON.stringify({
            id: id,
            statusCoordenador: status
        })
    };

    try {
        const request = await fetch('../Controller/Coordenador/StatusCoordenador.php', config);
        const response = await request.json();
